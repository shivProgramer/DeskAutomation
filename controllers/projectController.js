...
const getProjects = async (req, res) => {
  try {
    const projects = await ProjectCostDetail.findAll({
      ...
      include: [
        { model: AdminUser, as: "adminUser", attributes: ["user_id", "name"] },
...
async function createProject(req, res) {
  try {
    ...
    const newProject = await ProjectCostDetail.create({
      ...
      is_critical,
      project_manager_emails,
      user_id, // Add user_id when creating the project
    });
  ...
  try {
    ...
    if (start_date) {
      if (moment(start_date, "YYYY-MM-DD", true).isValid()) {
        ...
        filters[Op.and].push(
          Sequelize.where(
            Sequelize.cast(Sequelize.col("projectCostDetail.updated_at"), "DATE"), // Specify the alias here
            {
              [Op.gte]: start_date,
            }
          )
    ...
    if (stakeholder_email && stakeholder_email !== "") {
      filters.project_manager_emails = {
        [Op.like]: `%${stakeholder_email}%`, // Partial match for emails
      };
    }
